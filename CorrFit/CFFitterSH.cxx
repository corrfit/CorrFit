...
CFFitterSH::CFFitterSH() : CFFitter()
{
  ReadParameters();
  ...
  switch (mPType){
  case 1:
    mExpCF       = new ExpCFSH(EXPCFSH_SYSTEM_PP, sOutFile);
    break;
  case 2:
    mExpCF       = new ExpCFSH(EXPCFSH_SYSTEM_MM, sOutFile);
    break;
  case 3:
    mExpCF       = new ExpCFSH(EXPCFSH_SYSTEM_PM, sOutFile);
    break;
  case 4:
    mExpCF       = new ExpCFSH(EXPCFSH_SYSTEM_MP, sOutFile);
    break;
  case 5:
    mExpCF       = new ExpCFSH(EXPCFSH_SYSTEM_ZZ, sOutFile);
    break;
  case 6:
    mExpCF       = new ExpCFSH(EXPCFSH_SYSTEM_ZP, sOutFile);
    break;
  case 7:
    mExpCF       = new ExpCFSH(EXPCFSH_SYSTEM_ZM, sOutFile);
    break;
  case 8:
    mExpCF       = new ExpCFSH(EXPCFSH_SYSTEM_PZ, sOutFile);
    break;
  case 9:
    mExpCF       = new ExpCFSH(EXPCFSH_SYSTEM_MZ, sOutFile);
    break;
  default:
    PRINT_MESSAGE("Incorrect pair type charge " << mPType << " . Aborting");
    exit(1);
  }
  mCalcCF      = new CalcCFSH(((ExpCFSH *) mExpCF)->GetLowKStarFit(),
			      ((ExpCFSH *) mExpCF)->GetHighKSstarFit(),
			      ((ExpCFSH *) mExpCF)->GetNFitBin(),
			      ((ExpCFSH *) mExpCF)->GetLowKStarNorm(),
			      ((ExpCFSH *) mExpCF)->GetHighKStarNorm());
  ((CalcCFSH *) mCalcCF)->SetPrimaryFraction(mPrimaryFraction, mPrimaryFraction+331);
...
// 			     ((ExpCFSH *) mExpCF)->GetNFitBin()/2,
// 			     ((ExpCFSH *) mExpCF)->GetLowKStarFit(),
// 			     ((ExpCFSH *) mExpCF)->GetHighKSstarFit(),
// 			     50,-1.0,1.0);
...
void     
CFFitterSH::InitializePairManager()
{
  Int_t  tPCount;
  Pair  *tPair;
  Int_t *mNFPBuf;
...
//   ((CalcCFSH *) mCalcCF)->SetNormPBins(tNormP);
//   ((CalcCFSH *) mCalcCF)->SetNormNBins(tNormN);
...
void 
CFFitterSH::GeneratePairs(int *aPairsPerBin, int aNBins)
{
  if (!mGeneratePairs)
    return ;
...
//   TF1 *tFunP = ((ExpCFSH *) mExpCF)->mHDenOutPFit;  
//   TF1 *tFunN = ((ExpCFSH *) mExpCF)->mHDenOutNFit;
//   for (int iter=0; iter<4; iter++) {
//     PRINT_DEBUG_3("Par " << iter << " " << tFunP->GetParameter(iter) << " " << tFunN->GetParameter(iter));
//   }
//   Double_t tRangeMin = ((ExpCFSH *) mExpCF)->GetLowKStarFit();
//   Double_t tRangeMax = ((ExpCFSH *) mExpCF)->GetHighKSstarFit();
...
CFFitterSH::GetChi2()
{
  ...
  double tVal;
  for(int ti=0; ti<((ExpCFSH *) mExpCF)->GetNFitBin()*2; ti++){
...
double  
CFFitterSH::GetChi2(double aPurity)
{
  double tChi2=0.;
  const double* tContent = mCalcCF->GetContent();
  const double* tErr2 = mCalcCF->GetError2();
  const double* tContentExp = mExpCF->GetContent();
  const double* tErr2Exp = mExpCF->GetError2();
  const double* tCovarianceExp = ((ExpCFSH *) mExpCF)->GetCovariance();
  double tVal;
  //  double tScale;
  
  double se, see, me, mee, st, set, mt, met, cov;

  int tFitBins = ((ExpCFSH *) mExpCF)->GetNFitBin();
  ...
  if (0) {
    for(int ti=0; ti<((ExpCFSH *) mExpCF)->GetNFitBin()*2; ti++){
      ...
      if (isnan(tVal)) { PRINT_DEBUG_3("Is nan!"); continue; }
      ...
      PRINT_DEBUG_3("Chi bin " << 
		    ...
		    tErr2[ti] << " " << 
		    tVal << "    " <<
		    (tVal*tVal/(tErr2Exp[ti])) << "    " <<
		    tChi2);
    }
  }
  else {
    // Perform the calculation using the full covariance information
    
    for(int ti=0; ti<tFitBins; ti++){
      se = tContentExp[ti];
      me = tContentExp[ti+tFitBins];
      
      see = tErr2Exp[ti];
      mee = tErr2Exp[ti+tFitBins];

      st = (tContent[ti]-1)*aPurity+1.0;
      mt = tContent[ti+tFitBins]*aPurity;

      cov = tCovarianceExp[ti];

      tVal = (((st-se)*(st-se)/see +
	       (mt-me)*(mt-me)/mee -
	       2*cov*(st-se)*(mt-me)/sqrt(see*mee))/
	      (1-cov*cov));
...
void 
CFFitterSH::GenerateParameterStub()
{
  ...
  (*os) 
    << "# SH Fitter specific parameters" << endl 
    << "PairType = " << endl << endl;
  (*os) 
  ...
  (*os) 
    << "# Fraction of primary particles" << endl 
    << "PrimaryFraction = " << endl << endl;
  
  os->close();

  CalcCFSH::GenerateParameterStub();
  ExpCFSH::GenerateParameterStub();
  
}

void 
CFFitterSH::ReadParameters()
{
  ...
  try {
    ...
    PRINT_MESSAGE("CFFitterSH::ReadParameters - Error reading parameter: " << e.Data());
    exit(0);
  }
  try { 
    ...
    PRINT_MESSAGE("CFFitterSH::ReadParameters - Parameter not found: " << e.Data() << " Using default.");
    mRandomSeed = 21341;
  }
  try { 
    mPrimaryFraction = atof((sRPInstance->getPar("PrimaryFraction")).Data());
  }
  catch (STR e){
    PRINT_MESSAGE("CFFitterSH::ReadParameters - Parameter not found: " << e.Data() << " Using default.");
  ...
  try { 
    ...
    PRINT_MESSAGE("CFFitterSH::ReadParameters - Parameter not found: " << e.Data() << " Not generating additional pairs.");
    mGeneratePairs = 0;
  }
}
