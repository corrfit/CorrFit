...
CFStorageMySQL::CFStorageMySQL()
{
  ...
  PRINT_DEBUG_3("DB Parameters " << mDBHost.Data() << " " << mDBUser.Data() << " " << mDBPass.Data());
				   ...
				   int aPairCount)
{
  ...
  snprintf(tQString, 400, "SELECT fitParameterID FROM FitParameters WHERE fitBinsCount = %i AND fitMin='%.3lf' AND fitMax='%.3lf' AND normMin='%.3lf' AND normMax='%.3lf' AND momResScale='%.1lf'\0", aNFitBin, aFitMin, aFitMax, aNormMin, aNormMax, aMomResScale);
  ...
  if (!tNRows)                           
    ...
    {
      snprintf(tQString, 300, "INSERT INTO FitParameters (fitBinsCount, fitMin, fitMax, normMin, normMax, momResScale) VALUES (%i, '%.3lf', '%.3lf', '%.3lf', '%.3lf', '%.1lf')", aNFitBin, aFitMin, aFitMax, aNormMin, aNormMax, aMomResScale);
      ...
      snprintf(tQString, 300, "SELECT fitParameterID FROM FitParameters WHERE fitBinsCount = %i AND fitMin='%.3lf' AND fitMax='%.3lf' AND normMin='%.3lf' AND normMax='%.3lf' AND momResScale='%.1lf'\0", aNFitBin, aFitMin, aFitMax, aNormMin, aNormMax, aMomResScale);
