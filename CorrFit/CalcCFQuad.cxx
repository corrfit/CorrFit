...
CalcCFQuad::~CalcCFQuad()
{
  if (mContent) delete [] mContent;
  if (mErr2)    delete [] mErr2;
}

CalcCFQuad::CalcCFQuad(Double_t aFitLow, Double_t aFitHigh, Int_t aFitBin,
...
CalcCFQuad& CalcCFQuad::operator=(CalcCFQuad& aCalcCFQuad) 
{
  mPairManager = aCalcCFQuad.mPairManager;

  mNormP = aCalcCFQuad.mNormP;
  mNormN = aCalcCFQuad.mNormN;
  
  mFitLowKStar = aCalcCFQuad.mFitLowKStar;
  mFitHighKStar = aCalcCFQuad.mFitHighKStar;
  mNFitBins = aCalcCFQuad.mNFitBins;
  mBinWidth = aCalcCFQuad.mBinWidth;
  
  mNormLowKStar = aCalcCFQuad.mNormLowKStar;
  mNormHighKStar = aCalcCFQuad.mNormHighKStar;
}

short 
CalcCFQuad::InFitRange(Pair *aPair)
{
  return ((aPair->GetKStar() > mFitLowKStar) && (aPair->GetKStar() < mFitHighKStar));
}

short 
CalcCFQuad::InNormRange(Pair *aPair)
{
  return ((aPair->GetKStar() > mNormLowKStar) && (aPair->GetKStar() < mNormHighKStar));
}

int 
CalcCFQuad::FitRangeBin(Pair *aPair)
{
...
int 
CalcCFQuad::NormRangeBin(Pair *aPair)
{
  return 0;
...
void CalcCFQuad::Write()
{
}

//  void Read(TFile *aFile, TKey *aKey);
void CalcCFQuad::Normalize()
{
}

void CalcCFQuad::WriteParameters()
{
}

TGraphErrors *CalcCFQuad::GetGraph()
{
  STR tGName;
...
TGraphErrors *CalcCFQuad::GetGraph(double aPurity)
{
  STR tGName;
...
void CalcCFQuad::Calculate()
{
  ...
  // QuadRatio normalization by number of entries only
...
void CalcCFQuad::SetNormPBins(Int_t aBins) 
{
  mNNormPPair = aBins;
}

void CalcCFQuad::SetNormNBins(Int_t aBins)
{
  mNNormNPair = aBins;
}

int  
CalcCFQuad::ReadFromStorage()
{
  TVectorD *tC;
  TVectorD *tE;
