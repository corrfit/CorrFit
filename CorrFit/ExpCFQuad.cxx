...
ExpCFQuad::ExpCFQuad() : ExpCF()
{
  mPurityContent = 0;
}


ExpCFQuad::ExpCFQuad(int aChargeType,
...
{
  if (mPurityContent)
    delete mPurityContent;
}

TGraphErrors* ExpCFQuad::GetGraph(const char* aName)
{
}

TGraphErrors* ExpCFQuad::GetGraph(const char* aName, double aPur)
{
}

double ExpCFQuad::GetContent(int aIndex, double aPurity) const
{
  double tVar = mContent[aIndex];
  return (tVar-1.0) * aPurity +1.0;
}

double ExpCFQuad::GetPurity(int aIndex)
{
  if (mPurityContent)
    return mPurityContent[aIndex];
  else
    return 1.0;
}

void ExpCFQuad::WriteHisto()
{
}

void ExpCFQuad::Write()
{
  mHHCF->Write();
  mHCF->Write();
...
void ExpCFQuad::ApplyPurCorr(int aChargeType)
{
...
// 		}
	...
	PRINT_DEBUG_2("Readinng purity information");

	mPurityContent = (double *) malloc(sizeof(double) * mNFitBin);
	...
	for(int ti=0; ti<mNFitBin/2; ti++){
	  mPurityContent[ti] = ttPPur->GetBinContent(ttPPur->GetXaxis()->FindFixBin(mHCF->GetXaxis()->GetBinCenter(ti+tFirstFitBin)));
 	  PRINT_DEBUG_3("Purity bin value: " << mPurityContent[ti]);
	}
	
	tFirstFitBin = mHCF->FindBin(mLowKStarFit+tHalfStep);
	tLastFitBin = mHCF->FindBin(mHighKStarFit-tHalfStep);
	for(int ti=0; ti<mNFitBin/2; ti++){
	  mPurityContent[ti+mNFitBin/2] = ttPPur->GetBinContent(ttPPur->GetXaxis()->FindFixBin(mHCF->GetXaxis()->GetBinCenter(ti+tFirstFitBin)));
 	  PRINT_DEBUG_3("Purity bin value: " << mPurityContent[ti+mNFitBin/2]);
	}
      }
    // 	for(int ti=1;ti<=tHCF->GetNbinsX();ti++){
    // 	  tCorr = mPurCorr*ttPPur->GetBinContent(ti);
// 	  mPurityContent[ti-1] = mPurCorr*ttPPur->GetBinContent(ti);
...
void ExpCFQuad::ApplyMomResCorr(double aMomResCorr)
{
}

int ExpCFQuad::ReadCF(int aChargeType, TFile* aOutFile)
{
...
void ExpCFQuad::Build(int aChargeType)
{
  Normalize();
...
STR ExpCFQuad::MakeHistName(int aType, int aProj, int aSign, int aSystem)
{
  STR buff;
...
STR ExpCFQuad::MakePurityHistName(int aProj, int aSign, int aSystem)
{
  STR buff;
...
STR ExpCFQuad::MakeFileName(int aChargeType)
{
  STR fName;
  fName="";
  try
...
STR ExpCFQuad::MakePurityFileName(int aChargeType)
{
  STR fName;
  try
    {
...
void ExpCFQuad::SetParameters(int aNFitBin, double aLowKStarFit, double aHighKStarFit,
...
double ExpCFQuad::GetLowKStarFit()
{
  return mLowKStarFit;
}

double ExpCFQuad::GetHighKSstarFit()
{
  return mHighKStarFit;
}

double ExpCFQuad::GetLowKStarNorm()
{
  return mLowKStarNorm;
}

double ExpCFQuad::GetHighKStarNorm()
{
