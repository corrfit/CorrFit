		 ...
		 TFile* aOutFile) : ExpCF()
{
  ...
  if(tValid){
    PRINT_MESSAGE("Fits " << mLowKStarFit << " "<< mHighKStarFit);
...
void ExpCFSH::Normalize()
{
  ...
  int tiFirstScale = mRC00->FindBin(mLowKStarNorm);
  int tiLastScale = mRC00->FindBin(mHighKStarNorm);
  double tScale = mRC00->Integral(tiFirstScale,tiLastScale)/
    (tiLastScale-tiFirstScale+1);
  mRC00->Scale(1./tScale);
  
  //  cout << "Scale " << tScale << endl;

  double tScale11 = 0;
  
  for (int ib=tiFirstScale; ib<=tiLastScale; ib++)
    tScale11 += mRC11->GetBinContent(ib);
  tScale11 /= (tiLastScale-tiFirstScale+1);

  for (int ib=1; ib<=mRC11->GetNbinsX(); ib++) {
    mRC11->SetBinContent(ib, mRC11->GetBinContent(ib) - tScale11);
  }

  //  cout << "Scale 11 " << tScale11 << endl;
...
// 	}
      ...
      PRINT_MESSAGE("Text: " << (sRPInstance->getPar("FitRangeMin")).Data() << " Num " << (float) atof("0.005")<< " " <<mLowKStarFit);
      ...
      PRINT_MESSAGE("Ranges : " << mLowKStarFit << " "<< mHighKStarFit);
