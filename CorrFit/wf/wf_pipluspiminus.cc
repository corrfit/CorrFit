...
Twavefunction_pipluspiminus::Twavefunction_pipluspiminus(char *parsfilename){
  int iq,ichannel,*I;
  double q;
...
double Twavefunction_pipluspiminus::calcpsisquared(int iq,double r,
						  double ctheta){
  ...
  if(iq>=nqmax){
	...
	psi+=root2*0.5*ci*ci*5.0*legendre(2,ctheta)*hstar2
	  *(exp(-2.0*ci*delta_d)-1.0);
      }
	...
	psi+=root2*0.5*ci*3.0*legendre(1,ctheta)*hstar1
	  *(exp(-2.0*ci*delta_p)-1.0);
      }
